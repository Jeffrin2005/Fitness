...
function Overview({ userData }) {
  const [activities, setActivities] = useState(userData?.activity || {
    steps: 0,
    stepsGoal: 10000,
    runningKm: 0,
    runningGoal: 5,
  ...
  const [bodyMetrics, setBodyMetrics] = useState(userData?.bodyMetrics || {
    overall: 0
  })

  // Load data from localStorage on mount and listen for CSV updates
  useEffect(() => {
    ...
    if (savedActivityData) {
      try {
        const activityData = JSON.parse(savedActivityData)
    ...
    if (savedUserData) {
      try {
        ...
        if (userData.overall !== undefined) {
          setBodyMetrics({ overall: userData.overall })
        }
      } catch (error) {
    ...
    const handleCSVDataUpload = (event) => {
      ...
      if (csvData) {
        // Update activities if CSV contains activity data
        if (csvData.steps !== undefined || csvData.caloriesBurned !== undefined) {
        ...
        // Update body metrics if CSV contains body data
        if (csvData.overall !== undefined) {
          setBodyMetrics({ overall: parseInt(csvData.overall) || 0 })
        }
      }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-white via-orange-50/40 to-white rounded-2xl px-5 py-6 sm:px-8 sm:py-7 lg:px-10 lg:py-8 shadow-md border border-orange-200/30">
        <div className="relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          ...
          <div className="grid grid-cols-3 gap-3 text-xs sm:text-sm">
            <div className="rounded-xl bg-gradient-to-br from-white to-orange-50/50 backdrop-blur-sm border border-orange-200/40 px-3 py-2.5 shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-gradient-to-br from-white via-white to-gray-50/80 backdrop-blur-xl rounded-2xl p-6 shadow-md border border-gray-300/40">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <a href="/body-analysis" className="bg-gradient-to-br from-white to-gray-50/60 backdrop-blur-xl rounded-2xl p-6 shadow-md border border-gray-300/40">
          <div className="w-14 h-14 bg-gradient-to-br from-slate-700 to-slate-900 rounded-2xl flex items-center justify-center mb-4">
        ...
        <a href="/health-vitals" className="bg-gradient-to-br from-white to-red-50/40 backdrop-blur-xl rounded-2xl p-6 shadow-md border border-gray-300/40">
        ...
        <a href="/nutrition" className="bg-gradient-to-br from-white to-emerald-50/40 backdrop-blur-xl rounded-2xl p-6 shadow-md border border-gray-300/40">
