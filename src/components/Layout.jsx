...
function Layout({ children, userData, onLogout }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 via-gray-50 to-orange-50/30">
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full w-72 bg-gradient-to-b from-white via-white to-gray-50/80 backdrop-blur-xl border-r border-gray-300/50 shadow-xl transform transition-all duration-300 ease-in-out lg:translate-x-0 ${
              ...
              {navigation.map((item) => {
                ...
                return (
                  <Link
                    key={`tab-${item.path}`}
                    to={item.path}
                    className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-medium whitespace-nowrap border transition-all ${
                      isActive
